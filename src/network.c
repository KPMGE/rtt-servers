...
struct network {
	int quantityVertex, quantityEdges, quantityClients, quantityMonitors, quantityServers;
	int* idServers;
	int* idClients;
	int* idMonitors;
	LinkedList** linked;
...
Network* start_network(FILE* in) {
	...
	for (int i = 0; i < network->quantityEdges; i++) {
		...
		network->linked[idVertex1] = linkedList_insert(network->linked[idVertex1], vertex_start(idVertex2, cost));
	}
}
...
void read_network(FILE* in, Network* n, Distances* d) {
	...
	fscanf(in, "%d %d %d %d %d", &n->quantityVertex, &n->quantityEdges, &n->quantityServers, &n->quantityClients, &n->quantityMonitors);
	...
	d->vertexDistanceMonitors = (int*)malloc(sizeof(int*) * n->quantityMonitors);
